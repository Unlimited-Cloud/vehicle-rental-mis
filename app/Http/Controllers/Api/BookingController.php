...
class BookingController extends Controller
{
    ...
    public function createBookingNew(Request $request)
    {
        try {
            Log::info("Vehicle Bokking Request", ["body" => $request->all()]);
            ...
            $vehicleRate = DB::table('trip_routes_vehicle_price')
            ...
            $overnightCharge = (float) $vehicleRate->price;
    ...
    public function getVehiclePrice(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'vehicle_id' => 'required|exists:vehicles,id',
        ...
        $pricing = TripRouteVehiclePrice::where('vehicle_id', $request->vehicle_id)
            ->whereNull('trip_route_id')
            ->first();

        if (!$pricing) {
            return response()->json([
                'status' => 'error',
                'message' => 'Pricing not configured for selected vehicle.',
            ], 404);
        }
        ...
        $overnightCharge = ($request->boolean('overnight') && $pricing->overnight)
            ? $pricing->price
            : 0;
        ...
        return response()->json([
            ...
            'overnight_charge' => $pricing->price,
